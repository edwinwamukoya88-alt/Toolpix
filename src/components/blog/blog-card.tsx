...
export default function BlogCard({ post }: { post: BlogMeta }) {
  return (
    <Link
      href={`/blog/${post.slug}`}
      className="group relative flex flex-col rounded-2xl border bg-card/40 overflow-hidden transition-all duration-500 hover:-translate-y-1.5 hover:shadow-2xl hover:shadow-primary/10 hover:border-primary/40"
    >
      ...
      <div className="flex flex-col gap-3 p-5 flex-1">
        ...
        <p className="text-sm text-muted-foreground line-clamp-2 flex-1 leading-relaxed">{post.description}</p>
